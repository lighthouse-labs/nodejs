...
var breweryAPI = require('../brewerydb/');
...
slack.on('message',function(message){
    ...
    console.log(user);
    console.log(message.text.slice(14));
    //@U0BT5PHU4 is the name of the slack bot so you only respond when asked
    if(message.text.includes("@U0BT5PHU4")){
        breweryAPI(message.text.slice(14),function(error,data) {
            debugger;
            if (error)
                console.error(error);
            if (data)
                channel.send(data.name + ": " + data.description)
        });
...
slack.on('connect', function(){
    console.log('Slack Bot is Alive!');
